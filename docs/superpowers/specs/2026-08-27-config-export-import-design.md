...
세션이 도는 도중에 가져오더라도 빈 댓글은 나가지 않는다. 문구가 0개가 되면 `pickTemplate`이 `null`을 주고, 판정이 그 후보를 QUEUED에서 빼낸다 — 반쯤 채워진 문구가 나가지 않는 것과 같은 경로다. 대신 **그 세션은 옛 문구와 새 문구가 섞여 나갈 수 있다.** 문구는 후보마다 다시 읽히기 때문이고, 이는 문구 화면에서 세션 도중에 문구를 고쳤을 때와 같은 동작이다. 설계가 의도한 것이지 import가 새로 만든 상태가 아니다.
