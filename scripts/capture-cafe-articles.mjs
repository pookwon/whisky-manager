/**
 * Captures one Phase 0 article-list response through the extension's logged-in
 ...
 *
 * Run `pnpm capture:cafe-articles -- <page>` after quitting Whisky Manager,
...
} from '../dist/shared/cafeArticleFixture.js'
...
function pageFromArgs(args) {
  ...
  if (normalized.length !== 1 || !/^[1-9]\d*$/.test(normalized[0] ?? '')) {
    throw new Error('usage: pnpm capture:cafe-articles -- <positive-page-number>')
  }
...
async function main() {
  ...
  const url = cafeArticleListUrl(page)
  if (!isCafeArticleListTarget(url)) throw new Error('internal error: unsafe article-list target')

  const output = resolve(FIXTURES_DIRECTORY, `cafe-article-list-page-${page}.json`)
  ...
  try {
    ...
    if (reply.type !== 'PROBE_RESULT' || reply.error !== null || reply.status < 200 || reply.status >= 300) {
      throw new Error('article-list request failed; no fixture was written')
    }
...
main().catch((error) => {
  // An untrusted endpoint may include sensitive text in its error body; only
  // emit locally-created error messages, never the raw response or reply object.
