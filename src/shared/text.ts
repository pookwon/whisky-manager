...
export const TEXT = {
  ...
  collectionSettings: {
    ...
    interval: '수집 주기',
    intervals: {
      SIX_HOURS: { label: '6시간마다', hint: '하루 네 번. 새 글이 시간당 스무 건 안팎이라 한 번에 한두 쪽에서 끝납니다.' },
      TWELVE_HOURS: { label: '12시간마다', hint: '하루 두 번. 반나절치가 쌓여 대여섯 쪽을 읽습니다.' },
      DAILY: { label: '하루 한 번', hint: '하루치를 몰아서. 열 쪽 안팎이라 20쪽 휴지 전에 끝납니다.' },
      MANUAL: { label: '수동만', hint: '예약하지 않습니다. 수집 현황에서 직접 눌러 실행합니다.' },
    },
    baseAndRange: '기준 시각과 범위',
    baseTime: '기준 시각',
    rangeDays: '거슬러 읽을 범위',
    maxPages: '한 번에 최대',
    rangeDaysUnit: (days: number) => `최근 ${days}일`,
    maxPagesUnit: (pages: number) => `${pages}쪽`,
    grid: (times: string) => `기준 시각부터 주기마다 실행합니다 — ${times} (KST).`,
