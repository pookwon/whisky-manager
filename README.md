...
네이버 이용약관상 자동화 프로그램 이용은 위반 소지가 있으며 계정 이용 제한 가능성이 실재한다. 기술로 제거할 수 있는 리스크가 아니라 운영 정책으로 관리한다.

- 실행 계정은 **카페 운영 전용 계정**을 쓴다. 스태프 개인 계정을 쓰지 않는다
- 그 계정에 **카페 매니저 권한을 주지 않는다.** 정지되는 순간 카페 운영 자체가 마비된다
- 정지 시 도구를 전면 정지하고 수동 운영으로 되돌린다. **자동 계정 전환 기능은 만들지 않는다** — 제재 회피다
